    ...
    */
    ...
    if (props.registerErrors){ 
        props.registerErrors.forEach(
          error=>{
            switch (error.field) {
              case "non_field_errors":nonFieldErrors.push(error); break;
              case "email" :emailErrors.push(error) ; break ;
              case "username" : usernameErrors.push(error) ; break;
              case "password1": password1Errors.push(error) ; break;
              case "password2": password2Errors.push(error) ; break;  
              default: break;
            }          
          }
        )
